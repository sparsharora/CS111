...
void close_ssl(SSL* ssl_srv,SSL_CTX* ctx)
{
	SSL_free(ssl_srv);
	SSL_CTX_free(ctx);
}
...
ssl_ser=SSL_new(ssl_ctx);
SSL_set_fd(ssl_ser,sockfd);

if(SSL_connect(ssl_ser) != 1)
{
...
sprintf(temp,"ID=%d\n",id);
SSL_write(ssl_ser,temp,strlen(temp));
//dprintf(sockfd,"ID=%d\n",id);
...
while(1)
{
	...
	if(polls.revents & POLLIN)
	{
		...
		char *tok;
		ssize_t size;
		size=SSL_read(ssl_ser,temp,sizeof(temp)-1);
		//fprintf(stderr,"here\n");
